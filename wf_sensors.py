    ...
    def _reconcile_switch_states(self, st: Dict[str, Any]):
        ...
        def maybe_pub_bool(key: str, state_topic: str, cloud_val):
            ...
            if pend is not None:
                ...
                # 2) se il cloud conferma lo stato desiderato, chiudi il pending
                if cloud_b is not None and desired_b is not None and cloud_b == desired_b:
                    self.pending.pop(key, None)
                    if self.local:
                        self.local.publish(state_topic, b"ON" if cloud_b else b"OFF", qos=0, retain=True)

                # 3) pulizia pending scaduto → pubblica stato cloud reale
                elif now >= pend["until"]:
                    self.pending.pop(key, None)
                    if cloud_b is not None and self.local:
                        self.local.publish(state_topic, b"ON" if cloud_b else b"OFF", qos=0, retain=True)

                # altrimenti: grace period attivo, non sovrascrivere stato ottimistico

            else:
                # nessun pending: pubblica sempre lo stato cloud
                if cloud_b is not None and self.local:
                    self.local.publish(state_topic, b"ON" if cloud_b else b"OFF", qos=0, retain=True)
